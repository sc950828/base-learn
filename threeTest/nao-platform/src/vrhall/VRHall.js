...
export class VRHall {
  ...
  _init() {
    ...
    // this._renderer.toneMapping = THREE.ACESFilmicToneMapping; // 使用 ACES 色调映射
    this._renderer.toneMapping = THREE.NeutralToneMapping;

    // this._renderer.toneMappingExposure = 0.2; // 调整曝光
    ...
    rgbeLoader.load("../cannon_4k.hdr", function (texture) {
    ...
    this._controls.minZoom = 0.6;
    this._controls.maxZoom = 1;
    this._controls.minPolarAngle = Math.PI / 2; // 90 degrees
    this._controls.maxPolarAngle = Math.PI / 2; // 90 degrees
  ...
  moveTo(position, lookat, duration) {
    ...
    console.log("position, lookatV3", position, lookatV3);
  ...
  _createVideo(model) {
    this.video.src = "../飞书20240530-165408.mp4";
    this.video.loop = true;
    const texture = new THREE.VideoTexture(this.video);
    // 获取矩形平面Mesh(显示屏)设置map
    model.getObjectByName("SM_WanQiang_2").material =
  ...
  _createZhiBo(model) {
    ...
    iframe.onload = function () {
      console.log(iframe);
      var texture = new THREE.TextureLoader().load(iframe.src);
      ...
      document.getElementById("iframe").remove(); // 加载完成后移除 iframe
    };

    // let domEle = document.createElement("div");
    // domEle.innerHTML =
    //   "<div class='domBox'><iframe src='https://shangzhibo.tv/watch/11217431/?player' style='width: 100%; height: 100%; overflow: hidden' scrolling='no' allowfullscreen=''></iframe></div>";
    // /**
    ...
    // this._scene.add(domEleObj);

    // this.labelRenderer = new CSS3DRenderer();
    // this.labelRenderer.setSize(800, 600);
    // this.labelRenderer.domElement.style.position = "absolute";
    // this.labelRenderer.domElement.style.top = 0;
    // this._options.container.appendChild(this.labelRenderer.domElement);
